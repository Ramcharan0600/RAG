...
def sidebar_and_documentChooser():
    ...
    with tab_open_vectorstore:
        ...
        vectorstore_dir = Path("data/vector_stores")
        if vectorstore_dir.exists():
            vectorstores = [d.name for d in vectorstore_dir.iterdir() if d.is_dir()]
        else:
            vectorstores = []

        if vectorstores:
            selected_vectorstore_name = st.selectbox("Available Vectorstores:", vectorstores)
            clicked = st.button("Load Vectorstore")
        else:
            st.warning("No vectorstores found in data/vector_stores/")
            clicked = False
            selected_vectorstore_name = None

        st.session_state.selected_vectorstore_name = ""

        if clicked and selected_vectorstore_name:
            # Check inputs
            error_messages = []
            if (
            ...
            # if API keys are inserted, start loading Chroma index, then create retriever and ConversationalRetrievalChain
                ...
                selected_vectorstore_path = str(vectorstore_dir / selected_vectorstore_name)
...
def get_response_from_LLM(prompt):
    ...
    if "chain" not in st.session_state or st.session_state.chain is None:
        st.info("Please load or create a vectorstore before starting the chat.")
        return
